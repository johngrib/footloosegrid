...
// support functions
  ...
  function _curry_2_(func, param2) { 
    return function(param1) { return func(param1, param2) }; 
  }
   ...
   * @returns {String}
   */
  const _to_comma_format = (function() {
    const reg = /\B(?=(\d{3})+(?!\d))/g;
    return function (num) {
      if(num == null)
        return num;
...
function _scheme_initialize(scheme){
  ...
  scheme.forEach(function(col){
    ...
    column.data_push = (function data_push(){
      ...
      const formatter = column.input_formatter || function(v){ return v };
      const slicer    = column.input_slicer    || function(v){ return v };
      const caster    = column.input_caster    || function(v){ return v };
      const out_css   = column.output_css;
...
FGR.prototype.event_handler.change_val = function(e, evt_process) {
  ...
  const cell     = this.event_handler.cell = { editable: editable, loc: loc, };
...
function _create_cell_define(_this){
  ...
  //setter($cell, value, row, col, this);
  const cell_def     = {};
  const std_setter   = function ($cell,v) { return $cell.val(v) };
  const std_getter   = function ($cell  ) { return $cell.val( ) };
  const text_setter  = function ($cell,v) { return $cell.text(v) };
  const text_getter  = function ($cell  ) { return $cell.text( ) };
  const check_setter = function ($cell,v) { return $cell.prop('checked', _.isNumber(v) && v > 0) };
  const check_getter = function ($cell  ) { return $cell.prop('checked') ? 1 : 0; };
...
function _attatch_evt_excel(_this){
  ...
  function sheet_to_array(sheet, scheme) {
    ...
    const date_type_exist = _.some(scheme, function(col) { return ( col.type === 'date' ) });
    ...
    const cols      = col_range.map( function(c) { return XLSX.utils.encode_col(c) });

    let C, txt, val, col_index;
    const data = _.range(range.s.r, range.e.r + 1).map(function(Row) {
      const one_row = [];
...
function _create_rows(_this, data, callback){
  ...
  const inputs = _this.scheme.map(function(column, i) {
...
function _create_header(div_index, start, end){
  ...
  this.header_labels.forEach(function(v, row) {
    range.forEach(function(col) {
      ...
      const cell   = $('<div>',{row:row, col:col, col_merge: 1, row_merge: 1, 'class': _style.cell})
...
function _get_header_labels_array(){
  ...
  const _this  = this;
  const temp   = this.scheme.map(function(col) { return col.label.split('||') });
  const range  = _.range(_.max(temp.map(function(v) { return v.length })));
  const labels = range.map(function(r) { return [] });
...
function _create_proto_cell(element, mode){
  ...
  const cells = this.scheme.map(function(column, i) {
...
function _create_col_style(_this, col_index){
  ...
  _.range(col_index, _this.scheme.length).forEach(function(i) {
    ...
    const exp_body   = _.reduce(attr, function(before, current, key) { return ( before + key + ':' + current + ';') }, '');
    const exp_tail   = '}';
...
FGR.prototype.create_init_data = function(row_count){
  ...
  const row     = this.scheme.map(function(col) { return col.init_data });
...
function _create_buttons(_this){
  ...
  function drill(event){
    ...
    if(_this.data.length < _this.rows.length){
      ...
      const empty_rows = _this.create_init_data(last).map(function(row) { row.empty = true; return row; });
...
FGR.prototype.calc = {
  sum: function (_this, col_index) {
    const value = _this.data.reduce(function(a,b) { return ( a + b[col_index]) }, 0);
...
function _create_filter_icon(){
  ...
  this.scheme.forEach(function(column, col) {
    ...
    const attr   = { 'class': _style.filter_btn, col: col};
...
function _create_filter_div(){
  ...
  const div          = $('<div>', attr.div);
    ...
    const btn_run    = $('<button>', attr.run).click(  function() { return _this.run_filter(_this) } );
    const btn_clear  = $('<button>', attr.clear).click(  function() { return _this.clear_filter_condition(_this) } );
    const btn_close  = $('<button>', attr.close).click(  function() { return div.hide() } );
...
function _create_search_div(){
 ...
 ;[options.reg_exp.obj, options.reg_exp.title].forEach( function(obj) { obj[this.cfg.search_by_reg_exp ? 'show':'hide']() }.bind(this));
  ...
  options.reg_exp.obj.click(function(e){
    ...
    const disable = function (obj, flag) { return obj.attr('disabled', flag).css('opacity', flag ? 0.3 : 1) };
...
FGR.prototype.modal = function(text, button){
  ...
  const disable= function(d) { if(_.isBoolean(d)) _this.disable(d); };
  const func   = {
      obj : modal,
